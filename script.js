 ...
 * Paths account for GitHub Pages deployment at /Zoo/ subdirectory
 */
...
function setHeroImage() {
  if (heroImage) {
    heroImage.src = `${BASE_PATH}${HERO_IMAGE}`;
  }
}
  ...
  galleryGrid.innerHTML = ANIMALS.map(
    (animal, index) => `
      <article
        ...
        ${animal.hoverSound ? `data-hover-sound="${soundPath(animal.hoverSound)}"` : ""}
        tabindex="0"
      >
